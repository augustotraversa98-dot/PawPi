...
async function POST(request, { params }) {
  try {
    ...
    const saved = await sql`
      INSERT INTO event_rsvps (event_id, user_profile_id, pet_id, status)
      VALUES (${params.id}, ${userId}, ${petId}, ${status})
      ON CONFLICT (event_id, user_profile_id)
      DO UPDATE SET status = EXCLUDED.status, pet_id = EXCLUDED.pet_id, updated_at = now()
      RETURNING id, event_id, status, pet_id
    `;
