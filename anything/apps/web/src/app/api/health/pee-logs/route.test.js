...
// POST /api/health/pee-logs — night-run finding H1: the create handler previously trusted the
// client-supplied petId and inserted with owner_user_id = the caller, with NO check that the pet
// belonged to (or was shared with) the caller. It now routes through resolvePetLogOwner (the same
// owner-OR-family gate the food/walk/poo-log routes use), so a caller can only write against a pet
// they own or were granted family access to, and the row is anchored to the pet's true owner.
...
const post = (body) =>
  new Request("http://localhost/api/health/pee-logs", {
    method: "POST",
...
describe("POST /api/health/pee-logs — ownership gate", () => {
  ...
  it("cross-pet write (not owner, not family) → 403, no INSERT runs", async () => {
    ...
    const res = await POST(post({ petId: 999, frequency: "normal" }));
    expect(res.status).toBe(403);
    expect(resolvePetLogOwner).toHaveBeenCalledWith(7, 999);
    expect(sql).toHaveBeenCalledTimes(1); // profile lookup only — the write was blocked
  });
  ...
  it("owner write → 201, anchored to the resolved owner id", async () => {
    ...
    const res = await POST(post({ petId: 5, frequency: "normal" }));
