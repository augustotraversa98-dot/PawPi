...
async function GET(request, { params }) {
  try {
    ...
    // Batched in one query (same fix as api/shop/orders/route.js) instead of one round-trip
    // per program.
    ...
    for (const row of allProgress) {
      if (!progressByProgramId.has(row.program_id)) progressByProgramId.set(row.program_id, []);
      progressByProgramId.get(row.program_id).push(row);
    }
