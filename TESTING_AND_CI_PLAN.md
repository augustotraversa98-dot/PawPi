...
> **Status — R0 harness DONE (the RLS prerequisite).** Branch `ticket/rls-r0-pg-harness`. The ephemeral Postgres is `embedded-postgres` (a real Postgres binary run as a child process — **no Docker**, so it runs identically locally and on GitHub runners), booted once per run by `anything/apps/web/test/integration/globalSetup.ts`, which applies every `supabase/migrations/*.sql` in filename order and publishes the connection string via Vitest `provide()`/`inject()`. Run with `npm run test:integration` (separate `vitest.integration.config.ts` + a new `web (integration / real Postgres)` CI job). Delivered Phase C value now: real migrations + the airtight **jsonb write→read round-trip** (double-encode catch via `jsonb_typeof`), plus an **identity-chain + owner-scoping round-trip**. The fast mocked unit suite (`npm test`) is unchanged and stays the PR gate. **Deferred (noted, not done in R0):** the authenticated-route smoke against a real DB + session (A2.2 end-to-end) — full auth wiring is heavy; it rides this harness in a follow-up. R0 adds NO RLS, NO identity plumbing, NO app-code/`sql.js`/schema changes, NO Supabase migration. RLS follow-ups: R1 (per-request identity via `set_config` through a central seam) → R2 (`pawpi_app` role + policies + FORCE RLS) → R3 (prod cutover).
