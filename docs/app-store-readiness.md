...
|---|---|---|
| iOS permission strings | 5.1.1, 5.1.5, 2.5.14 | Added honest `NS*UsageDescription` for every permission the app actually uses — `NSLocationWhenInUseUsageDescription` (places/walks/transport/events + live pet-taxi GPS), `NSCameraUsageDescription` (pet photos / health checks / posts), `NSPhotoLibraryUsageDescription` + `NSPhotoLibraryAddUsageDescription` (uploads + saving the share frame), `NSMicrophoneUsageDescription` (telehealth video). No permission is declared that the app doesn't use; **no ATT / `NSUserTrackingUsageDescription`** (the app does not track users across apps). `anything/apps/mobile/app.json`. **Localized EN+ES (PP2):** `expo.locales` → `anything/apps/mobile/locales/{en,es}.json` + `CFBundleLocalizations: ["en","es"]`, so the OS prompt itself is in the user's language; `locales/locales.test.js` pins EN/ES key parity and keeps the English file identical to `ios.infoPlist` (the base fallback). |
...
5. **Support URL (required for submission). ✅ RESOLVED (2026-07-31).** The
   ...
   `pawpi-legal` repo + GitHub Pages mechanism as Privacy/Terms; published 2026-06-29). `pawpi.info`
   itself still has no web hosting configured (parked at one.com; DNS/hosting for the root domain is
   a one.com account action Code cannot perform), so `docs/app-store-connect-content.md` §6 has been
   updated to point the Support URL field at the github.io URL directly (mirrors the already-accepted
   Privacy Policy precedent) instead of waiting on DNS. If `pawpi.info` is ever pointed at real hosting
