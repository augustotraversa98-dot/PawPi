...
| In-app account deletion | 5.1.1(v) | **Added** (separate PR): Settings → "Delete account" → typed/confirmed → `DELETE /api/account` irreversibly deletes the caller's account + owner-scoped data, then signs out. Owner-scoped + harness-proven. See that PR for the RLS proof. |
