  ...
  items.map((item) => () =>
    agent(
      [
        ...
        '   validate with `npx npm@10 ci --dry-run` before committing it. For DB/RLS work also run',
        '   `npm run test:integration` (real-Postgres harness) and add as-pawpi_app zero-rows proofs.',
        '4. Append a dated entry to docs/test-backlog.md (under "## To test") for this item: a short',
        '   plain-English "what shipped" + the concrete phone-checkable steps, and — if you created a',
        '   migration — a "MIGRATION TO APPLY TO SUPABASE: <file>" line. Commit it with your change.',
        '5. Commit (clear message), push the branch, and open a PR with `gh pr create` (NOT draft).',
        '   PR body: what changed, the conventions honored, the on-device checklist, and any migration note.',
        '6. Do NOT merge. Do NOT touch files outside this item\'s scope (other agents build in parallel).',
        '',
